...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(c -> c, Collectors.counting())).entrySet()
                .stream()
                .filter(p -> p.getValue() > 1).count();
    }
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min((album1, album2) -> {
            Track defaultTrack = new Track("", 0);

            Function<Album, Integer> getMaxRatingInAlbum = album -> (album.getTracks().stream().
                    max((track1, track2) -> ((Integer)track1.getRating()).compareTo(track2.getRating()))).
                    orElse(defaultTrack).getRating();

            return (getMaxRatingInAlbum.apply(album1)).compareTo(getMaxRatingInAlbum.apply(album2));
        });
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted((album1, album2) -> {
            Function<Album, Double> getAverageRatingInAlbum =
                    album -> album.getTracks().stream().mapToInt(Track::getRating).average().orElse(0);
            return getAverageRatingInAlbum.apply(album2).compareTo(getAverageRatingInAlbum.apply(album1));
        }).collect(Collectors.toList());
    }
    ...
    public static String joinTo(String... strings) {
        return Arrays.stream(strings).collect(Collectors.collectingAndThen(
